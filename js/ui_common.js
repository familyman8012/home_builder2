...
$('body').smartscroll(function(){
    var header = $('.header');
    if (header.hasClass('over') && $('body').scrollTop() != 0) {
        header.removeClass('over').addClass('over_not');
    } else if (header.hasClass('over_not') && $('body').scrollTop() == 0) {
        header.removeClass('over_not').addClass('over');
    }
});
...
function gridAreaCreate() {
    ...
    newGridsort = [];
    for (var i = 0; i < $('.wrap_grid').length; i++) {
        newGridsort[i] = new Sortable(eval('grid' + (i+1)), {
            animation: 150,
            swapThreshold: 1,
...
function includeLayout2() {
    ...
    $.each(includeArea, function () {
        self = $(this);
        url = self.data("include"); 
         if (!$('#list1').hasClass('wrap_fullpage') && $('.wrap_grid').length != 0) {
	            for (var i = 0; i < $('.wrap_grid').length; i++) {
	                newGridsort[i].destroy();
	            }
	            //newGridsort.destroy();
        	}       
...
$(document).ready(function () {
    $(document).on('mousedown', 'section', function (e) {
        ...
        e.stopPropagation();
        if ($(this).closest('section').hasClass('main_visual')) {
        ...
        }).on('mousedown', 'header .txt_area', function (e) {
        e.stopPropagation();
        rightlogoTxtEdit($(this));
    }).on('dblclick', 'header .txt_area', function (e) {
        e.stopPropagation();
       dbclickTxtingEdit($(this));
...
$(document).on({
    mouseenter: function (e) {
        e.stopPropagation();
        $(this).append('<span class="icon_i btn_el_setting"><i class="blind">설정버튼</i></span>');
    },
    mouseleave: function (e) {
        e.stopPropagation();
       $('.btn_el_setting').remove();
    }
}, "section, .img_area, .txt_area, .btn_area, .logo, .login.on, .wrap_search.on, .gnb, .main_visual, footer, .wrap_images_area, .wrap_movie_area, .mask_youtube, .wrap_mainmovie_area .mask_mvis, .wrap_map_area, .wrap_gallery");


$(document).on('click', 'section .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuSection($(this));
}).on('click', '.img_area .btn_el_setting', function (e) {
    e.stopPropagation();
    rightImgEdit($(this));
}).on('click', 'section .txt_area .btn_el_setting', function (e) {
    e.stopPropagation();
    if ($(this).closest('section').hasClass('main_visual')) {
        rightMenuMainVis($(this));
    } else {
...
}).on('click', 'footer .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuFooter($(this));
}).on('click', 'footer .txt_area .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuTxtEdit($(this));
}).on('click', '.wrap_txt_area .btn_el_setting', function (e) {
    e.stopPropagation();
    rightTxtingEdit($(this));
}).on('dblclick', 'section .txt_area .btn_el_setting', function (e) {
    e.stopPropagation();
    dbclickTxtingEdit($(this));
}).on('dblclick', 'footer .txt_area .btn_el_setting', function (e) {
    e.stopPropagation();
    dbclickTxtingEdit($(this));
})
    .on('click', '.btn_area .btn_el_setting', function (e) {
        e.stopPropagation();
        rightMenuBtnEdit($(this));
    }).on('click', 'header .logo .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenulogoEdit($(this));
}).on('click', 'footer .logo .btn_el_setting', function (e) {
    e.stopPropagation();
    rightFooterlogoEdit($(this));
}).on('click', '.login.on .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenulogin($(this));
}).on('click', '.wrap_search.on .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuSearch($(this));
}).on('click', '.gnb .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuHeader($(this));
}).on('click', '.main_visual .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuMainVis($(this));
}).on('click', '.wrap_images_area .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuImage($(this));
}).on('click', '.wrap_movie_area .btn_el_setting, .mask_youtube .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuVideo($(this));
}).on('click', '.wrap_mainmovie_area .mask_mvis .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuMainVideo($(this));
}).on('click', '.wrap_map_area .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuMap($(this));
}).on('click', '.padding_sty2 .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuPadding($(this));
}).on('click', '.wrap_gallery  .btn_el_setting', function (e) {
    e.stopPropagation();
    rightMenuGallery($(this));
}).on('mousedown', 'header .txt_area', function (e) {
        e.stopPropagation();
...
function rightMenuSection(el) {
    ...
    if ((event.button == 2) || (event.which == 3) || $(event.target).hasClass('btn_el_setting')) {
...
$('.btn_all_menu').on('click', function () {
    ...
    $(".manage_option_page").append($('<div>').load("./include/popup.html .edit_all_menu", function () {
        $('.mask').show();
        if (!$('#list1').hasClass('wrap_fullpage')) {
	        menuSetting();
	   		newSort.destroy();
...
function headStySel() {
    ...
    $('.set_header .btn_apply').on('click', function () {
        $('.mask').hide();
        $('.set_header').remove();
        if (document.querySelector('.edit-mode .header').classList.contains('vertical')) {
            document.getElementById('html_main').setAttribute('style','margin-top:100px');
        }
    });
}
...
function logoSetting(el) {
    ...
    targetlogo.removeClass('off_icon_i');
    ...
    $('.logo .wrap_txt_area').hide();
}

function logoTxtSetting() {
    $('.logoimg').hide();
    $('.logo .wrap_txt_area').show();
    $('.header .logo').addClass('off_icon_i');
}

function logoDelSetting(el) {
    $('.header .logo').removeClass('off_icon_i');
    el.closest('.logo').find('.wrap_txt_area').hide();
...
function menuSetting() {
   ...
   //메뉴 선택시 ID값 입력
    ...
    $(document).on('click', '.edit_all_menu .btn_del', function () {
        //$("#js_menu_name").text(del_menu_name);
        $('.pop_area').load("./include/popup.html .pop_menudel", function () {
            if ($('.depth1_li').hasClass('on')) {
                var menuNamePop = $('.on .txt .menu_name').text();                
            } else if ($('.depth2_li').hasClass('on')) {
                var menuNamePop = $('.on .txt2 .menu_name').text();
            }
            $('.alert_txt').prepend(menuNamePop);
        });
        includeLayout();
    });
    ...
    $(document).on('click', '.depth1_li', function (e) {
        e.stopPropagation();
    ...
    $(document).on('click', '.depth2_li', function (e) {
        e.stopPropagation();
    ...
    function thisMenuSave(el) {
     ...
     //home 값 지정하기
    var home_href  = $(".box_add_depth2").attr('href');
    var home_name = $(".menu_name").val();
    $(".page_link").val(home_href);
...
$(document).on('click', '.edit_all_menu .btn_apply', function () {
    ...
    (["data-on='on'"]).addClass('on').removeAttr('data-on');
    $('.gnb .inner .atag').each(function () {

        var elhref = $(this).attr('href');
        //동우추가 메뉴이름데이터넣기
        var menu_name = $(this).find('em').text();
        console.log($(this).parent());
        $(this).parent().attr('data-name', menu_name);
        $(this).parent().attr('data-href', elhref);
        var eltarget = $(this).attr('target'); // 문제점
        if (eltarget == 'undefined' || eltarget == undefined) {
            eltarget = '_self';
        }
        ...
        //$(this).contents().unwrap().wrap('<a href="' + elhref + '" class="' + elClass + ' atag"></a>');
    });    
    ...
    $('.mask').hide();
    $('.gnb').find('[data-on="on"]').addClass('on').removeAttr('data-on');
...
function depth1menu_add() {
	//menu_cnt = $('.wrap_pop').find(".depth1_li").length + 1
    //var menunName = $(".menu_name").text();
    //console.log(menunName);
    var menu_cnt = makeRandom(1,999999);
    var createMenu = '<li class="depth1_li" ><span href="/thema/basic/empty.html?menu=' + menu_cnt + '" target=""  class="txt box_add_depth2 atag"><span class="wrap_menu"><em class="menu_name">신규메뉴</em><i class="btn_add"><span class="blind">하위 메뉴추가 버튼</span></i></span></span>'
    createMenu += '</li>'
    $('.edit_all_menu').find('.on').removeClass('on');
    $('.edit_all_menu .list_depth1').append(createMenu);
    $('.menu_area').scrollTop($('.list_depth1 li').length * 120);
}
function submenu_add(el) {
 	//menu_cnt = $('.wrap_pop').find(".depth2_li").length + 1
    var menu_cnt = makeRandom(1,999999);
    if (el.closest('li').find('.list_depth2').length == 0) {
        el.closest('li').append('<ul class="list_depth2 nested-sortable"></ul>');
    }
    //var menuName = $(".depth1_li").filter('.on').find('em.menu_name').text();
    //console.log(menuName);
    el.closest('li').find('.list_depth2').append('<li class="depth2_li" ><span href="/thema/basic/empty.html?menu=' + menu_cnt + '" target="" class="txt2 atag"><span class="wrap_menu"><em class="menu_name">신규서브메뉴</em></span></span></li>');

}


//메뉴 체크
$(document).on("click", ".edit_all_menu .btn_add", function (e) {
    menu_cnt = $('.wrap_pop').find(".depth1_li").length  //대 메뉴
    sub_cnt = $('.wrap_pop').find(".depth2_li").length  //서브메뉴
    total_cnt = menu_cnt + sub_cnt;
    mb_lv = $("#mb_lv").val();
    ...
    if (mb_lv == '0') {
        if (total_cnt >= 5) {
            ...
            if (e.target.parentNode.className == 'box_add_depth1') {
                depth1menu_add()
            } else {            
               submenu_add($(this))
            }
...
function currentUrlMenu() {
    var currentUrl = window.location.href.split("play-block.com")[1];
    var currentUrl2 = window.location.href.slice(-7);
     $('.gnb a').each(function(index, el) {
        if ($(this).attr('href') == currentUrl) {
            var menuFindName = $(this).find('.menu_name').text();
             $('.this_menu_name').text(menuFindName);           
        } else if ( currentUrl2 == "&edit=Y") {
            $('.this_menu_name').text($('.gnb li').eq(0).find('.txt .menu_name').text());    
        }
    }); 
     return currentUrl;
...
function menuOneSetting() {
    var currentUrl = currentUrlMenu();
    $('#menu_name').val($('.this_menu_name').text());
    var el = $('.gnb .on a');
    if (!(el.attr('href').slice(0,6) == '/thema')) {
        $('#url_link').val(el.attr('href').split("https://")[1]);
    } else {
        $('#page_link').val(currentUrl);
    }
    if (el.hasClass('off')) {
    ...
    console.log($('.gnb .on a').attr('target'));
    if ( $('.gnb .on a').attr('target') == '_blank') {
    ...
    $(document).on('click', '.edit_one_menu .btn_apply', function () {
        ...
        if ($('#url_link').val() == '' || $('#url_link').val() == undefined) {
            el.attr('href', $('#page_link').val());
        } else {
            el.attr('href', 'https://' + $('#url_link').val());
        }
        if ($('.displayis').is(":checked")) {
            $('.gnb .on a').addClass('off');
        } else {
            $('.gnb .on a').removeClass('off');
        }
       
        if ($('.newWindow').is(":checked")) {
            $('.gnb .on a').attr('target', '_blank');
        } else {
            $('.gnb .on a').removeAttr('target');
        }
    ...
    if (!($(this).find('.list_depth2').length == 0)) {
        ...
        $(this).find('.list_depth2').slideToggle('fast', function () {
            ...
            if ($('.depth1_li.on').length == 0) {
                $('.gnb .depth1_li[data-on="on"]').addClass('on').removeAttr('data-on');
            }
...
function h_builder_common(targetel) {
    ...
    $('#font_family').change(function (event) {
        ...
        if (ffVal == 'font') {
            ...
            $('#html_main').append('<div class="set_fontfamily"><link rel="stylesheet"  href="css/' + ffVal + '.css"></div>');
        }
    ...
    $('.com_set .btn_apply').on('click', function () {
        ...
        if (bodyColorSetting == undefined) {
            ...
            $('header, main').css('background-color', bodyColorSetting);
        }
...
function headOver() {
    $('header').addClass('over');
     $('#html_main').css('margin-top', '50px');
}

function headOverCancle() {
    $('header').removeClass('over');
     $('#html_main').css('margin-top', '130px');
}
function headOverInit() {
    if ($('.edit-mode .header').hasClass('over')) {
        $('#html_main').css('margin-top', '50px');
    } else {
        $('#html_main').css('margin-top', '130px');
    }
    ...
    function handleImgFileSelect(e) {
        ...
        filesArr.forEach(function (f) {
            ...
            reader.onload = function (e) {
                ...
                if ($(".add_slide_area .wrap_txt").length == 1) {
                    $(".add_slide_area .wrap_txt").addClass('now');
                }
                index++;
            }
...
function sectionBg(targetSection) {
    targetSection.addClass('bgsetting');
     if ($('.bgsetting').css('background') != '') {
        $('.pop_section_bg .pop_cont').addClass('prevBg').css('background', $('.bgsetting').css('background'));
    }
    ...
    function handleImgFileSelect(e) {
         $('.pop_section_bg  .pop_cont').removeClass('prevBg').css('background', '');
...
function summernoteDsetting() {
    ...
    var saveTxtEditing = function (el) {
        ...
        $('.wrap_txt span, .note-editable span').css('display','');
        el.summernote('destroy');
        if (!$('#list1').hasClass('wrap_fullpage')) {
            newSortList(list1);
        	gridAreaCreate();
        }
          $('.wrap_txt_area.on, .inner_section.on').removeClass('on');
    };
...
function textEdit(nowpos) {
    ...
    if (!$('#list1').hasClass('wrap_fullpage') && $('.note-popover').length == 0) {
        newSort.destroy();
    }
    if (!$('#list1').hasClass('wrap_fullpage') && $('.wrap_grid').length != 0) {
            console.log($('.wrap_grid').length);
             console.log(newGridsort[1]);
            for (var i = 0; i < $('.wrap_grid').length; i++) {
                newGridsort[i].destroy();
                console.log(i);
            }
        ...
        $('.note-popover').closest('.wrap_txt_area, .inner_section').addClass('off').removeClass('on');
        nowpos.closest('.wrap_txt_area, .inner_section').addClass('on').removeClass('off');
            $('.off .txt_area').summernote('destroy');        
           
            $('.wrap_txt_area.off, .inner_section.off').removeClass('off');
...
function imgEdit($imgeditTarget, urledited) {
    ...
    $image_crop.croppie({
     ...
     $image_crop.croppie('bind', editImgSrc).then(function(){ 
...
function footerSetting() {
    ...
    for (var i = 0; i <= snsIconLength; i++) {
        ...
        if ($snsShareInfo[i] != undefined) {
            ...
            sns_share_clone += ' 	<li class="icon-sns1"><span class="blind">플레이콕계정으로 이동하기</span></li>'
            sns_share_clone += ' 	<li class="icon-sns2"><span class="blind">페이스북계정으로 이동하기</span></li>'
            sns_share_clone += ' 	<li class="icon-sns3"><span class="blind">네이버블로그</span></li>'
            sns_share_clone += ' 	<li class="icon-sns4"><span class="blind">네이버 블로그계정으로 이동하기</span></li>'
            sns_share_clone += ' 	<li class="icon-sns5"><span class="blind">유튜브계정으로 이동하기</span></li>'
            sns_share_clone += ' </ul>'
            sns_share_clone += '</div>'
    ...
    $('.pop_edit_footer .btn_close').one('click', function () {
        $('footer').removeAttr('class').addClass(currentFooterStyle);
        $('.pop_edit_footer').remove();
        $('.mask').hide().removeClass('mask_footer');
    });
}
...
function fillInAddress() {
    ...
    m_position = {lat:place.geometry.location.lat(),lng:place.geometry.location.lng()}
    var map = new google.maps.Map(document.getElementById("map"), {
        zoom: 15,
        center: m_position
    });
    var marker = new google.maps.Marker({position: m_position, map: map});
}

function mapLoadSetting(el) {
    var $MapAreaTarget = el.closest('.wrap_map_area').addClass('setting_area_map');
    var db_api_key = $("#db_api_key").val();

    $(".pop_area").load("./include/popup.html .set_map", function () {

        $('.mask').show();
        $('.set_map').prepend('<script src="https://polyfill.io/v3/polyfill.min.js?features=default"></script>\n' +
            '    <script\n' +
            '            src="https://maps.googleapis.com/maps/api/js?key='+db_api_key+'&callback=initMap&libraries=places&v=weekly"\n' +
            '            defer></script>\n' +
            '    <script>\n' +
            '        "use strict";\n' +
            '\n' +
        ...
        $(document).on('click', '.wrap_api_inp a', function(){
            var pop_area2 = document.createElement('div');
            pop_area2.classList.add('pop_area2');            
            $('.pop_area').after(pop_area2);
            $(pop_area2).load('/include/map_api_guide.html', function(){
...
function mapLoadSetting2() {
    var $MapAreaTarget = $('.setting_area_map');
    var db_api_key = $("#api_key").val();
    ...
    $(".pop_area").load("./include/popup.html .set_map", function () {
        ...
        mapElSetting($MapAreaTarget);
        $('.wrap_api_inp').attr('style','display:none;');
     $('.cont_area').show();
     $('.wrap_adrs_inp').show();
...
function videoElSetting($videoAreaTarget) {
    ...
    function videoElPreview() {
        if (($('#urlmovie').val() != ' ')) {
            ...
            if ($('#urlmovie').val().indexOf('you') != -1) {
                ...
                $urlmovieVal= $urlmovieVal.substring($urlmovieVal.length, $urlmovieVal.length -11);
    ...
    $('.complete').on('click', function () {
       ...
       if ($('#urlmovie').val().indexOf('play') != -1) {
...
function mainVideoElSetting($MainvideoAreaTarget) {
    ...
    if ($MainvideoAreaTarget.find('video').length != 0) {
        ...
        var bgThumbImg = mVideoSrc.replace('/board/', '/thumnail/').replace('mp4', 'jpg');
        $('.main_set_movie .edit_area').summernote(summernoteDefalutSetting);
    ...
    function mainVideoPreview() {
        if (($('#urlmovie').val() != ' ')) {
            ...
            } else if ($('#urlmovie').val().indexOf('play') != -1) {
                console.log('bb');
                var $urlmovieVal = $('#urlmovie').val().replace('/board/', '/thumnail/').replace('mp4', 'jpg');
    ...
    $('.file_multi_video').change(function () {
        ...
        movie_save(this.files[0],2,$MainvideoAreaTarget);
    });
    $('.complete').on('click', function () {
        ...
        if (($('#urlmovie').val() != ' ')) {
            if ($('#urlmovie').val().indexOf('you') != -1) {
                ...
                var $urlmovieVal = $('#urlmovie').val();
                $urlmovieVal= $urlmovieVal.substring($urlmovieVal.length, $urlmovieVal.length -11);
                $MainvideoAreaTarget.empty().append('<div class="mask_mvis"></div><iframe width="100%" height="100%" src="//www.youtube.com/embed/' + $urlmovieVal + '?rel=0&autoplay=1&amp;loop=1;&amp;controls=0&amp;mute=1&amp;playlist=' + $urlmovieVal + '" frameborder = "0" allowfullscreen = "" ></iframe>');
                ...
                var $urlmovieVal = $('#urlmovie').val();
                $MainvideoAreaTarget.empty().append('<div class="mask_mvis"></div><video style="width:100%;" muted autoplay loop><source src="' + $urlmovieVal + '" class="mainvideo_init"></video>');
                $('.main_set_movie').remove();
            }
        }
...
$(document).on('click', '.mask .btn_close, .wrap_pop  .btn_close', function () {
    if ($('.wrap_pop').hasClass('pop_visual_setting')) {
        ...
        $('.add_slide_area, .pop_visual_setting .note-editable, .pop_visual_setting .edit_area').empty();
        $('.pop_visual_setting').remove();
    } else if ($('.mask').hasClass('imgLightbox') || $('.mask').hasClass('movie')) {
        $('.mask').hide();
        $('.mask').removeClass('imgLightbox movie');
        $('.lightInImg').remove();
...
/* 컴포넌트 추가 */
$(document).on('click', '.component_add .btn_add, .empty_page', function (e) {
...
//mj 작성 - 경우에 따라 3번째 eventFn 인자 넘겨서 사용하시면 됩니다. 
function alert_load(text, btns, eventFn) {
    var maskdiv = $('body').prepend('<div class="alert_mask"></div>');

    var pop_wrap = document.querySelector('.pop_area');
    
    $(pop_wrap).load('/include/alert.html', function(){    
        var alert_wrap = pop_wrap.querySelector('.common_alert');

        $(alert_wrap).find('.txt_alert').text(text);

        if(btns == '1') {
            $(alert_wrap).find('.btn_cnxl').hide();
            $(document).on('click', '.btn_cnfm', function(){
                $(pop_wrap).html('');
                $('.alert_mask').remove();
            });
        } if(btns == '2') {
            $(document).on('click', '.btn_cnxl', function(){
                $(pop_wrap).html('');
                $('.alert_mask').remove();
            });
        }
