...
function sectionMoveDown(nowpos) {
    event.stopPropagation();
    if ($('#list1').hasClass('wrap_fullpage')) { 
        var addSlideNumber = $('.section.active').index() + 2;   
...
function textEdit(nowpos) {
    ...
    summernoteTraget.summernote({
        ...
        fontNames: ['본고딕','Arial', 'Arial Black', 'Comic Sans MS', 'Courier New'],
        fontSizes : ['11', '12', '13', '14', '16', '18', '20', '22', '24', '26', '28', '30', '36', '44', '48', '64', '82', '150'],
        focus     : true,
        popover   : {
            link     : [
...
function videoElSetting($videoAreaTarget) {
    if ($videoAreaTarget.hasClass('autoplay')) {
        $('#autoplay').prop("checked", true);
    }
    if ($videoAreaTarget.hasClass('lightbox')) {
        $('#lightboxchk').prop("checked", true);
    }
    if ($videoAreaTarget.closest('.div_movie_el').hasClass('fullvideo')) {
    ...
    if ($videoAreaTarget.find('.vjs-tech').attr('src') != undefined) {
        ...
        if ($('.vjs-tech').attr('src').indexOf('www') != -1) {
    ...
    if ($videoAreaTarget.find('iframe').length != 0 && $videoAreaTarget.find('.vjs-tech').attr('src') == undefined) {
    ...
    $('.complete').on('click', function () {
        ...
        if ($('#urlmovie').val() != ' ') {
            if ($('#urlmovie').val().indexOf('you') != -1) {
                ...
                if ($('#autoplay').is(":checked") == true) {
                    $('.wrap_movie_area').addClass('autoplay');
                    $('.' + $runVideojs).attr('muted', 'muted').attr('autoplay', 'autoplay');
                } else {
                    $('.wrap_movie_area').removeClass('autoplay');
                    $('.' + $runVideojs).removeAttr('muted', 'muted').removeAttr('autoplay', 'autoplay');
                }
            }
        } else {
            $videoAreaTarget.empty().append($videoClone2);
        ...
        if ($('#fitmovie').is(":checked") == true) {
            $videoAreaTarget.closest('.div_movie_el').addClass('fullvideo');
        } else {
            $videoAreaTarget.closest('.div_movie_el').removeClass('fullvideo');
        }
